...
_GT_POLY_ROOT ='//Salmon/User/Chen/Vessel_data/Dataset for Distance Assessment/crop_vessels_pelvis_36cases/seperation_left_right/Polygons'
...
_FIG_TGT = '//Salmon/User/Chen/Vessel_data/Dataset for Distance Assessment/crop_vessels_pelvis_36cases/seperation_left_right/Polygons/PolyFigures_Rim_20230214'
...
_Dis_TGT = '//Salmon/User/Chen/Vessel_data/Dataset for Distance Assessment/crop_vessels_pelvis_36cases/seperation_left_right/Polygons/PolyDistances_20230214'
_Dis_TGT_vessel = '//Salmon/User/Chen/Vessel_data/Dataset for Distance Assessment/crop_vessels_pelvis_36cases/seperation_left_right/Polygons/PolyDistances_Vessel_20230214'
...
_Dis_TGT_index = '//Salmon/User/Chen/Vessel_data/Dataset for Distance Assessment/crop_vessels_pelvis_36cases/seperation_left_right/Polygons/PolyDistances_pelvis_id'
    ...
    try:
        ...
        for i in range(len(Pelvis_index)):
            ...
            for j in range(cell_ids.GetNumberOfIds()):
                ...
                pelvis_model.GetCellPoints(cell_id, cell_points)

                # Loop through the points in the cell
                for k in range(cell_points.GetNumberOfIds()):
                    # Get the point ID
                    point_id = cell_points.GetId(k)

                    # Set the color of the point depending on the minimum distance
                    if min_distance < 15.0:
                        ...
                        colors.InsertNextTuple3(0, 255, 255)
                    elif min_distance < 35.0:
                        colors.InsertNextTuple3(0, 255, 255)
                    elif min_distance < 40.0:
                        colors.InsertNextTuple3(0, 255, 128)
                    else:
                        colors.InsertNextTuple3(0, 255, 0)

            # Add the colors array to the pelvis patch data
            pelvis_patch.GetPointData().SetScalars(colors)

        # Visualize the pelvis patch
        patch_mapper = vtk.vtkPolyDataMapper()
        patch_mapper.SetInputData(pelvis_patch)
