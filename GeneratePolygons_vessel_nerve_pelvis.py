...
_WIN_SIZE = [500,500]
_LABELS_F = './structures_json/osaka_pelvis_lr.json'
...
with open (_LABELS_F, 'r') as f:
    _LABELS = json.load(f)
_OUT_EXT = '.vtk'        
_IN_EXT = 'label_lr.mhd'
_ROOT = '//Salmon/User/Chen/Vessel_data/vessels_with_muscles'
_TGT = '//Salmon/User/Chen/Vessel_data/vessels_with_muscles/seperation_left_right/Polygons'
...
# _FILE_LIST = '//Salmon/User/Chen/Vessel_data/Nara_enhance_plain/crop_cases_list.txt'
_FILE_LIST = '//Salmon/User/Chen/Vessel_data/Dataset for Distance Assessment/crop_vessels_pelvis_36cases/caseid_list_vessels_36.txt'
...
def process_image(in_img_path, pg, tgt = 'Pelvis'):
    # _case = os.path.basename(in_img_path)[0:5]
...
if __name__ == "__main__":
    ...
    # Build class
    # pg = PolygonGenerator(out_dir= 'tests_polygon_generator')
    # test_file = './samples/17035395872_1210170749246166_4.mhd'
    ...
    for idx,lbl_file in enumerate(tqdm.tqdm(label_files)):
        # out_file_path = '%s/%s' % (_TGT, os.path.basename(os.path.dirname(lbl_file)).replace('.mhd', _OUT_EXT))
    ...
    tgts = list(_LABELS.keys())[0:8]
    for tgt in tgts:
        Parallel(n_jobs = _N_JOBS)(delayed(process_image)(img, pg, tgt) for img in \
                                    tqdm.tqdm(selected_label_files))
