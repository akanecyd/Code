...
fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(30, 35))
...
com_color = {"Institute 1": 'paleturquoise', "Institute 2": 'lightpink'}
...
palette={'Institute 1 GT Distance(mm)':'white', 'Institute 1 Predicted Distance(mm)':'white',
         'Institute 2 GT Distance(mm)': 'aquamarine','Institute 2 Predicted Distance(mm)': 'aquamarine'}
box = sns.boxplot(x='vessel/side', y=_STRUCT, data=_Distance_box, ax=ax, hue='Institute+Label',palette = palette,
          linewidth=5,
                  showmeans=True, showfliers = False, width= 0.8, hue_order = ['Institute 1 GT Distance(mm)',
         'Institute 1 Predicted Distance(mm)', 'Institute 2 GT Distance(mm)', 'Institute 2 Predicted Distance(mm)'],
                  meanprops={"marker": "s", "markerfacecolor": "white", "markeredgecolor": "blue",
                             "markersize": 10.0})
hatches = ['', '/', '', '/']
for i, patch in enumerate(ax.artists):
    hatch = hatches[i % len(hatches)]
    patch.set_edgecolor('black')# 计算每个组的填充图案
    patch.set_hatch(hatch)
    patch.set_edgecolor('black')
male_box = _Distance_box[_Distance_box['gender'] =='male']
palette={'Institute 1 GT Distance(mm)':'b', 'Institute 1 Predicted Distance(mm)':'b','Institute 2 GT Distance(mm)': 'b','Institute 2 Predicted Distance(mm)': 'b'}
sns.stripplot(x='vessel/side', y=_STRUCT, data=male_box, color='b', size=25, hue='Institute+Label',linewidth=2.5,
              hue_order=
              ['Institute 1 GT Distance(mm)',
           'Institute 1 Predicted Distance(mm)', 'Institute 2 GT Distance(mm)', 'Institute 2 Predicted Distance(mm)'],
              jitter=True, dodge=True, marker='.', edgecolor='b', alpha=0.8, ax=ax)
for i, box in enumerate(ax.artists):
    box.set_edgecolor('black')
famale_box = _Distance_box[_Distance_box['gender'] =='famale']
palette={'Institute 1 GT Distance(mm)':'firebrick', 'Institute 1 Predicted Distance(mm)':'firebrick','Institute 2 GT Distance(mm)': 'firebrick','Institute 2 Predicted Distance(mm)': 'firebrick'}
sns.stripplot(x='vessel/side', y=_STRUCT, data=famale_box, color='r', size=25, hue='Institute+Label',
              hue_order=
              ['Institute 1 GT Distance(mm)',
           'Institute 1 Predicted Distance(mm)', 'Institute 2 GT Distance(mm)', 'Institute 2 Predicted Distance(mm)'],
              palette = palette,
...
for _EXP in _EXPs:
  for _LABEL in _LABELs:
    sub_dat = _Distance_box [(_Distance_box ['Institute'] == _EXP) & (_Distance_box ['GT/Auto'] == _LABEL)]
    row = ['%.3f±%.3f mm' % (sub_dat[sub_dat['vessel/side']== _STR][_STRUCT].mean(skipna=True),
                          sub_dat[sub_dat['vessel/side'] == _STR][_STRUCT].std(skipna=True))
           for _STR in _TMP_STRUCT_NAMES]
    cell_text.append(row)
...
for _x, _STR in enumerate(_TMP_STRUCT_NAMES):
    for _EXP in _EXPs:
      sub_df = _Distance_box[(_Distance_box['vessel/side'] == _STR)& (_Distance_box ['Institute'] == _EXP)]
    # print(sub_df[sub_df['w/wo muscles'] == _EXPs[0]]['Points Distance(mm)'].values)
    # print(sub_df[sub_df['w/wo muscles'] == _EXPs[1]]['Points Distance(mm)'].values)
      [u_test_val, p_val] = stats.ttest_rel(sub_df[sub_df['GT/Auto'] == _LABELs[0]][_STRUCT].values,
                    sub_df[sub_df['GT/Auto'] == _LABELs[1]][_STRUCT].values,
                    nan_policy='omit')
...
for gender in genders:
  for _LABEL in _LABELs:
    sub_dat = _Distance_box [(_Distance_box ['gender'] == gender) & (_Distance_box ['GT/Auto'] == _LABEL)]
    row = ['%.3f±%.3f mm' % (sub_dat[sub_dat['vessel/side']== _STR][_STRUCT].mean(skipna=True),
                          sub_dat[sub_dat['vessel/side'] == _STR][_STRUCT].std(skipna=True))
           for _STR in _TMP_STRUCT_NAMES]
    cell_text.append(row)
...
the_table = plt.table(cellText=cell_text,
                      colLabels=_TMP_STRUCT_NAMES,
                      rowLoc='center',
                      rowLabels=['Institute 1 GT', 'Institute 1 Auto', 'Institute 2 GT','Institute 2 Auto','Male GT','Male Auto','Female GT', 'Female Auto'],
                      # rowColours=colors,
                      colColours=[[0.85, 0.4, 0.31, 0.65],
                                  [0.0, 0.59, 0.81, 0.65],
                                  [0.96, 0.84, 0.19, 0.65]],
...
print (cell_text)
# for i, cell in enumerate(the_table.get_celld()[1].values()):
#     cell.set_facecolor(colors[i])
#     cell.set_hatch(hatches[i])
ax.set_xticklabels('')
ax.set_xlabel('')
...
plt.ylim(-0.5, max(_Distance_box[_STRUCT])+5)
plt.ylabel(' Minimum distance ; mm',fontsize=50,fontweight='bold')
plt.title(' Minimum distance Distribution of GT-Auto', fontsize=50,fontweight='bold')
...
fig.savefig(os.path.join(_TGT_FIG,  'GT_AUto_Mindistance_boxplot_with_gender.png'))
# #
#
# ##   variable      value
# ## 0     leaf   9.446465
